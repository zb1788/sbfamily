...
//				log.info("[DATASOURCE    PASSWORD       ]:"+dbconfig.getProperty("password"));
				...
				String ip = "192.168.151.126";
				String port = "3306";
				String username = "root";
				String password = "123456";
				Properties dbconfig = new Properties();
				
				String url = "jdbc:mysql://"+ip+":"+port+"/badmin?useUnicode=true&characterEncoding=utf-8&useAffectedRows=true&serverTimezone=UTC";
				dbconfig.setProperty("driverClassName","com.mysql.jdbc.Driver");
				dbconfig.setProperty("url",url);
				dbconfig.setProperty("username", username);
					...
					if(coltypename.startsWith("timestamp") || coltypename.startsWith("date")){
						try{
							if(rs.getDate(collab)!=null){
								resmap.put(collab, DateUtils.parseDate(new Date(rs.getTimestamp(collab).getTime()),"yyyy-MM-dd HH:mm:ss"));
							}
						}catch(Exception e){
	        ...
	        for (Object[] vol:paramValues) {
	        	...
	        	for(Object param : vol){
	        		if (param instanceof Integer) {
	        			...
	        			//2019-10-10 sqlDate只保留到年月日，timestamp可保存年月日时分秒
	        			Timestamp st = new Timestamp(d.getTime());
	        			pstmt.setTimestamp(voi, st);
	        		   }else{
                     ...
                     try {
                    	 ...
                    	 if(paramtype.equals("java.lang.String")){
                    		 ...
                    		 if(DateUtils.parseDate(value)!=null){
                        		 m.invoke(obj, new Object[]{DateUtils.parseDate(value)});
                    		 }
                    	 }
